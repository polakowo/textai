...
def train(args, train_dataset, model, tokenizer):
    """ Train the model """
    if args.local_rank in [-1, 0] and not args.lr_finder:
    ...
    for epoch in range(int(args.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            # CHANGE: Do not forget attention masks for BERT
            attn_masks = (inputs != 0).type(torch.LongTensor).to(args.device)
            outputs = model(inputs, masked_lm_labels=labels, attention_mask=attn_masks) if args.mlm else model(inputs, labels=labels)
